...
GameMenuUI::GameMenuUI(SDL_Renderer* renderer, int windowWidth, int windowHeight, GameManager* manager) :
	renderer(renderer), selectedButton(0), backgroundTexture(nullptr) ,
	manager(manager)
...
void GameMenuUI::createButtonTexture(Button& button) {
	...
	SDL_Color colorTop = button.isHovered ? SDL_Color{255, 200, 100, 255} :
	                     SDL_Color{150, 180, 220, 255};
	SDL_Color colorBottom = button.isHovered ? SDL_Color{255, 160, 50, 255} :
	                        SDL_Color{100, 140, 200, 255};
			...
			if (SDL_PointInRect(&mousePoint, &button.rect)) {
				...
				switch (selectedButton) {
					case 0:
						...
						manager->setState(GameState::BattleScreen);
